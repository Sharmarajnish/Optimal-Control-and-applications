...
opts['minE'] = 1
opts['fixedT'] = True
neuron_ampl(24,4,x0,dx0,opts)

def neuron_ampl(N,T,x0,dx0,options):
...
#	ampl += 'param dx0 {states};\n'
	...
	#ampl += 'subject to initialConditionD {j in states}: (2/T)*(sum{k in nodes} D[1,k]*x[j,k]) = dx0[j];\n\n'
	#ampl += 'subject to initialConditionD {j in states}: (x[j,2]-x[j,1])/(times[2]-times[1]) = dx0[j];\n\n'
...
#	ampl += array2ampl('param','dx0',dx0)
	...
	return ampl, 'neuron_jdauwels', 'N;Tmax;x0;xT;T;eps;C;h;tau;x;u;cost',''